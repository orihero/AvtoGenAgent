            ...
            <Modal isOpen={isOpen}>
                <View style={styles.addOrderModal}>
                    <View style={styles.modalHeader}>
                        <Text style={{fontSize: 20, fontWeight: 'bold'}}>Add new order</Text>
                    </View>
                    <View style={styles.form}>
                        ...
                        >
                            {
                                carTypes.map(carType => <Picker.Item label={carType.title} value={carType.id}/>)
                            }
                        </Picker>
                        <View style={styles.services}>
                            {
                                services.map(service =>
                                    <View style={{flexDirection: 'row', marginBottom: 20}}>
                                        <TouchableNativeFeedback onPress={() => checkBoxHandler(service.id)}>
                                            <Text style={{width: '80%', marginRight: 20}}>{service.title}</Text>
                                        </TouchableNativeFeedback>
                                        <CheckBox
                                            isChecked={selectedServices[service.id]}
                                            onClick={() => checkBoxHandler(service.id)}
                                        />
                                    </View>
                                )
                            }
                        ...
                        <View style={{flexDirection: 'row', justifyContent: 'center', marginTop: 30}}>
                            <Text style={{...styles.btn, backgroundColor: colors.accent}}
                                  onPress={() => setIsOpen(false)}>Cencel</Text>
                            <Text style={{
                                ...styles.btn,
                                backgroundColor: colors.yellow,
                                color: colors.accent
                            }} onPress={submitFormHandler}>Submit</Text>
                        </View>
                    </View>
                </View>
            </Modal>
